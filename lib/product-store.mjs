...
async function readGoogleProductRows() {
  ...
  return rows.slice(1).map((row,index)=>({
    rowNumber:index+2,
    product: normalizeRow(Object.fromEntries(headers.map((header,headerIndex)=>[header,row[headerIndex]??''])))
  })).filter(record=>record.product.id || Object.values(record.product).some(value=>value!==null && value!==''));
}
async function readGoogleProducts() {
  return (await readGoogleProductRows()).map(record=>record.product);
}
...
export async function updateProduct(id,product) {
  assertWritable(); const records=await readGoogleProductRows(); const record=records.find(item=>item.product.id===id);
  if(!record){const error=new Error('Product not found');error.code='PRODUCT_NOT_FOUND';throw error;}
  const tab=process.env.GOOGLE_SHEETS_TAB||'Products',rowNumber=record.rowNumber,next=normalizeRow({...record.product,...product,id});
...
export async function deleteProduct(id) {
  assertWritable(); const tab=process.env.GOOGLE_SHEETS_TAB||'Products'; const records=await readGoogleProductRows();
  const record=records.find(item=>item.product.id===id);
  if(!record){const error=new Error('Product not found');error.code='PRODUCT_NOT_FOUND';throw error;}
  const sheetId=await getSheetId(tab);
  await googleRequest(':batchUpdate',{method:'POST',body:JSON.stringify({requests:[{deleteDimension:{range:{sheetId,dimension:'ROWS',startIndex:record.rowNumber-1,endIndex:record.rowNumber}}}]})});
