...
import { withProductMutationLock } from './product-mutation-lock.mjs';
...
async function getSheetId(tab) {
  ...
  const sheet=(data.sheets||[]).find(item=>item.properties?.title===tab);
  if(!sheet) { const error=new Error(`Google Sheets tab not found: ${tab}`); error.code='PRODUCT_STORAGE_NOT_CONFIGURED'; throw error; }
...
export async function createProduct(product) {
  return withProductMutationLock(async () => {
    assertWritable();
    const tab=process.env.GOOGLE_SHEETS_TAB||'Products'; const next=normalizeRow(product);
    if (!next.id) { const error=new Error('Product id is required'); error.code='PRODUCT_INVALID'; throw error; }
    const products=await readGoogleProducts();
    if (products.some(item=>item.id===next.id)) { const error=new Error('Product id already exists'); error.code='PRODUCT_ID_CONFLICT'; throw error; }
    await googleRequest(`/values/${encodeURIComponent(`${tab}!A:Q`)}:append?valueInputOption=USER_ENTERED&insertDataOption=INSERT_ROWS`,{method:'POST',body:JSON.stringify({values:[flattenProduct(next)]})});
    return next;
  });
}
...
export async function deleteProduct(id) {
  return withProductMutationLock(async () => {
    assertWritable(); const tab=process.env.GOOGLE_SHEETS_TAB||'Products'; const products=await readGoogleProducts();
    const index=products.findIndex(item=>item.id===id);
    if(index<0){const error=new Error('Product not found');error.code='PRODUCT_NOT_FOUND';throw error;}
    const sheetId=await getSheetId(tab);
    await googleRequest(':batchUpdate',{method:'POST',body:JSON.stringify({requests:[{deleteDimension:{range:{sheetId,dimension:'ROWS',startIndex:index+1,endIndex:index+2}}}]})});
