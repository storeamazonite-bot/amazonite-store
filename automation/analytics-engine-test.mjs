...
test('rejects direct, nested, and disguised PII before storage', () => {
  for (const payload of [
    { product_id: 'prod-123', email: 'visitor@example.com' },
    { product_id: 'prod-123', meta: { phone: '+123456789' } },
    { product_id: 'prod-123', cta_label: 'contact visitor@example.com' }
  ]) {
    const store = memoryStore();
    const engine = createAnalyticsEngine({ store });
    assert.throws(() => engine.track('product_view', payload), AnalyticsValidationError);
...
test('does not fail the storefront when local storage is unavailable or quota-limited', () => {
  const unavailable = createLocalStore(null, 'test');
  assert.doesNotThrow(() => unavailable.append({ type: 'product_view' }));

  const quota = {
    getItem() { return '[]'; },
    setItem() { throw new Error('QuotaExceededError'); }
  };
  const store = createLocalStore(quota, 'test');
  assert.doesNotThrow(() => store.append({ type: 'product_view' }));
});
...
test('keeps analytics explicitly local until a central sink is configured', () => {
  const store = memoryStore();
  const engine = createAnalyticsEngine({ store });
  const event = engine.track('ai_interaction', { interaction: 'product_recommendation' });
  assert.equal(event.source, 'local');
...
test('supports an explicit central sink without inventing aggregation', () => {
  ...
  const central = memoryStore();
  const engine = createAnalyticsEngine({ store: local, centralSink: central });
  const event = engine.track('affiliate_click', { product_id: 'prod-9' });

  assert.equal(event.source, 'central');
  assert.equal(local.read().length, 1);
  assert.equal(central.read().length, 1);
  assert.deepEqual(central.read()[0], local.read()[0]);
  assert.equal(engine.isCentralized(), true);
});

test('does not silently convert a central sink failure into a false success', () => {
  const local = memoryStore();
  const central = { append() { throw new Error('central unavailable'); } };
  const engine = createAnalyticsEngine({ store: local, centralSink: central });

  assert.throws(() => engine.track('product_view', { product_id: 'prod-1' }), /central unavailable/);
  assert.equal(local.read().length, 1);
});
